(() => {
  if (!window.render || !window.renderMatch || !window.getSummary || !window.matches || !window.state || !window.els) return;

  state.matchday = Number(sessionStorage.getItem("prmMatchday") || "1");

  window.renderMatchdays = function renderMatchdaysTabbed(summaries) {
    const matchdays = [
    ...
    (active?.items || []).forEach((item) => grid.appendChild(window.renderMatch(item)));

    shell.append(tabs, header, grid);
    return shell;
  };

  window.render = function renderTabbed() {
    const summaries = matches.map(getSummary);
    const visibleMatches = summaries.filter(matchesFilter);

    if (els.matchGrid) {
      els.matchGrid.innerHTML = "";
      els.matchGrid.appendChild(window.renderMatchdays(visibleMatches));
    }
